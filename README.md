...
The Jira config file lives at [`src/AviAI.Server/scripts/jira/config.jira.json`](src/AviAI.Server/scripts/jira/config.jira.json) and currently stores the default Jira host `https://avicadocts.atlassian.net/`.
The current ticket reader script still requires `-BaseUrl`, so pass the same hostname from the config file when reading a ticket.
...
```powershell
.\src\AviAI.Server\scripts\jira\Get-JiraTicket.ps1 -BaseUrl "https://avicadocts.atlassian.net" -IssueKey "WID-712"
```

3. Add `-Raw` if you want the full Jira response object instead of the summarized ticket fields.

```powershell
.\src\AviAI.Server\scripts\jira\Get-JiraTicket.ps1 -BaseUrl "https://avicadocts.atlassian.net" -IssueKey "WID-712" -Raw
```
