...
10.2. No change to `AviAI.Server` or `AviAI.Client.Lib` is complete without a corresponding xUnit test in `AviAI.Tests` that uses Moq to isolate dependencies.
10.3. Tests must be written for new code and updated for modified code.
10.4. There are no exceptions.

## 2. Jira Assistant Protocol

1. If the user says `I need Jira assistance`, switch into `jira_assistant` mode.
2. In `jira_assistant` mode, help the user read Jira tickets and perform Jira-related repository tasks.
3. When the user asks to read a Jira ticket, use the PowerShell script at `src/AviAI.Server/scripts/jira/Get-JiraTicket.ps1`.
4. Use the `JIRA_APIKEY` environment variable as the Jira API secret for Jira automation.
5. Require the Jira base URL and issue key when reading a ticket unless the user has already provided them.
6. Keep `README.md` aligned with Jira assistant usage and Jira script changes.
...
10. Use PowerShell for repository scripts.
10.1. Do not add Bash, CMD, or Python scripts for repository automation unless the user explicitly asks for a different scripting language.
